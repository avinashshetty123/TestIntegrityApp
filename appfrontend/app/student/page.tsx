...
import StudentMeeting from "../../components/studentmeeting/StudentMeeting";

export default function StudentPage() {
  const router = useRouter();
  const [currentView, setCurrentView] = useState<
    "dashboard" | "meetings" | "proctored" | "join-code"
  >("dashboard");
  ...
  const fetchStudentData = async () => {
    try {
      ...
      const meetingsResponse = await fetch(
        "http://localhost:4000/meetings/visible",
        {
          credentials: "include",
        },
      );
      ...
      const availableTestsResponse = await fetch(
  ...
  const quickActions = [
    ...
    },
    {
      ...
      label: "System Check",
      onClick: () => router.push("/student/compatibility"),
    },
  ];
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-orange-100 to-white text-gray-800 flex items-center justify-center">
        <div
          className="text-center p-8 rounded-3xl bg-white/60 backdrop-blur-3xl border border-orange-200/60 shadow-2xl"
          style={{ boxShadow: "0 40px 80px rgba(251, 146, 60, 0.25)" }}
        >
      ...
      <section className="max-w-5xl mx-auto mb-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            >
              Take tests, join meetings, and track your progress in a secure,
              AI-monitored environment.
            </motion.p>
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">
                {userData?.institutionName || "Complete your profile"}
              </div>
            </div>
          </motion.div>
        </div>
            ...
            availableTests.map((test: any, i) => (
              ...
              >
                <div className="text-sm text-orange-600 font-medium mb-2">
                  {test.creator?.institutionName || "Institution"}
                </div>
                      ...
                      try {
                        const response = await fetch(
                          `http://localhost:4000/meetings/${meeting.id}/join`,
                          {
                            ...
                            credentials: "include",
                            body: JSON.stringify({
                              displayName: userData?.firstName || "Student",
                            }),
                          },
                        );
